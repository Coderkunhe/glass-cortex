...
graph TD
    ...
    subgraph 不能做["❌ 明确不能做"]
        ...
        NOMULTI["多模型协作<br/>（仅单一 deepseek-chat）"]
    end
...
**第二类：不能做但不知道自己不能做**——系统没有多模型协作（只有一个 deepseek-chat）、没有置信度自动校准（q8.1）、没有递归自我审查（q8.3）。但这些"不能做"没有被显式记录为能力边界的"负数空间"。一个真正的自画像应该同时包含"我能做什么"和"我不能做什么"。
