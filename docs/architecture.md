...
| Risk Assessment — `docSearch.ts` | ✅ 完成 | 2026-08-08 | 🟡 `docSearch.ts` (4C·1L — flattenDocs/createDocSearchIndex/DocSearchResult). 4 消费者: AdminShell(flattenDocs) · DocsPanel(createDocSearchIndex+flattenDocs) · SearchModal(createDocSearchIndex+DocSearchResult) · docSearch.test.ts. 品类：类型常量/工具函数 — 纯函数无状态无副作用无异步。Fuse.js 为唯一外部依赖，threshold(0.4)/keys 权重(0.6+0.4) 为模块内部配置不暴露给消费者。变更风险极低：新增搜索字段或调整权重不影响消费者签名。接口变更（如新增必选参数）需同步 3 个生产消费者 — Phase 1000 B141 |
| Risk Assessment — `AdminSidebar.tsx` | ✅ 完成 | 2026-08-08 | 🟡 `AdminSidebar.tsx` (4C·1L — AdminTab type + 默认导出组件). 4 消费者: AdminShell(组件+AdminTab) · DocsPanel(AdminTab type) · AdminSidebar.test.tsx(组件+AdminTab) · DocsPanel.test.tsx(AdminTab type). 品类：UI 组件 — ① Props 接口(5 props: activeTab/mobile/onTabChange/onCollapse/onOpenSearch) ② 内部状态(collapsedGroups Set + MENU_GROUPS 静态数据) ③ 渲染输出(品牌标识区+可折叠分组+激活态 brand token) ④ 错误/空/边界态(空 MENU_GROUPS 返回空 nav · mobile prop 切换 Drawer 模式) ⑤ 消费者组合(mobile Drawer vs 桌面固定侧栏 · onOpenSearch 可选回调). 变更风险低：新增 AdminTab 值需同步 AdminShell handleTabChange + DocsPanel filterGroup。Props 新增可选字段向后兼容 — Phase 1000 B141 |
| Risk Assessment — `RequirementsLogPanel.tsx` | ✅ 完成 | 2026-08-08 | 🟡 `RequirementsLogPanel.tsx` (3C·1L — extractLatestPhase + 默认导出组件). 3 消费者: AdminShell(组件) · RequirementsLogPanel.test.tsx(组件) · extractLatestPhase.test.ts(extractLatestPhase). 品类：UI 组件 — ① Props 接口(1 prop: onSelectDoc) ② 内部状态(当前日志内容+归档列表+四态: loading/error/empty/data) ③ 渲染输出(当前日志 brand 卡片+"当前"徽章+历史归档表 Phase 倒序) ④ 错误/边界态(API 失败降级 · getDocContent null 兜底 · extractLatestPhase null 兜底→FALLBACK_CURRENT_PHASE 66) ⑤ 消费者组合(AdminShell 通过 onSelectDoc 回调与 DocViewer 编排). 变更风险极低：单一 prop 接口 · extractLatestPhase 为正则纯函数无副作用 · 新增消费者仅需符合 onSelectDoc 回调签名 — Phase 1000 B141 |
...
| Windows Server 部署文档完善 + 打包脚本修复 | ✅ 完成 | 2026-08-08 | `build-package.sh` 三修：① standalone 展平 `standalone/frontend/*` → `standalone/*`（消除 server.js 路径嵌套）② frontend rsync 排除 test-results/data/.claude/Bn ③ `deploy/README.md` 新增 `--patch` 增量模式文档 + `Cannot find module` 排错条目 — Phase 67 Batch 18 |
| 打包补 tools/ + .env.example 拉通对齐 | ✅ 完成 | 2026-08-08 | `build-package.sh` SOURCE_DIRS 补 tools/（admin/health 依赖 check_docs.py）· `.env.example` 新增 `NEXT_PUBLIC_ADMIN_PASSWORD` + 删除死配置 `GLASSCORTEX_DATA_DIR` `LOG_LEVEL` — Phase 67 Batch 19 |
| check_docs.py 跨平台崩溃修复 | ✅ 完成 | 2026-08-08 | `_run_ruff()` 跨平台 ruff 路径定位（`sys.executable` + `shutil.which` 兜底）+ `_run_python_tool()` OSError 捕获 + `main()`/`_main_json()` check_fn() try/except 防御 — Phase 67 Batch 20 |
| patch 模式补 docs/ 增量同步 | ✅ 完成 | 2026-08-08 | check_docs.py `--changed-only` 增量模式（`CHECK_SKIP_PATTERNS` + `_any_changed` fnmatch）+ `.env.example` 补 `NEXT_PUBLIC_API_URL` + `CORS_ORIGINS` + Makefile `check-docs-patch` 目标 — Phase 67 Batch 21 |
